# Boostlook CSS — Modular Source

## Overview

`boostlook.css` is the Boost Look CSS framework, built from modular source files in `src/css/`. The modules replaced an earlier hand-maintained monolith of the same name; the filename is the stable contract every consumer pins, so it did not change.

The production `boostlook.css` is generated by concatenating all modules in order via `build-css.sh`. It is a build artifact and is committed; never edit it directly.
...
This concatenates all modules (in numeric order) into `boostlook.css` at the repo root.
